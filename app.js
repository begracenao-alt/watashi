...
(function () {
  ...
  const MODES = {
    ...
    trial30: {
      ...
      summary: true, // 30日の最後に「あなたの30日まとめ」を出す（売り込みCTAなし）
    },
    member: { limit: 'none' },
  };
  ...
  function logResult(type, mood) {
    try {
      const log = JSON.parse(lsGet('bg_log_free') || '{}');
      log[todayStr()] = { type: type.id, mood: mood }; // 同じ日は上書き＝1日1件
      lsSet('bg_log_free', JSON.stringify(log));
    } catch (e) {}
  }
  // 「あなたの30日まとめ」を組み立てる（開始日以降の記録だけ集計）
  function buildSummaryHtml() {
    let log = {};
    try { log = JSON.parse(lsGet('bg_log_free') || '{}'); } catch (e) {}
    let startStr = '';
  ...
  function showLocked() {
    ...
    if (cfg.summary) {
      // 30日モード：最後に「あなたの30日まとめ」を出す（売り込みなし）
